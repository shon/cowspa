...
@syntax('> (div.*)')
def SELF_CLOSING_DIV(m):
    text = m.group(1).strip()
    return '<%s> </div>' % apply_jquery(text)[0]
...
TAG_START_PAT = "[\s]*<[\s]*([\S]*)[\s]*>[\s]*"
TAG_END_PAT =   "[\s]*<[\s]*/[\s]*([\S]*)[\s]*>[\s]*" # unused
EXCLUDE_PROCESSING_TAGS = ['script', 'style']

def exclude_tag_end_check(line):
    return '/script' in line
    match = re.match(TAG_END_PAT, line)
    if match:
        return match.groups()[0].lower()

def exclude_tag_start_check(line):
    return '<script' in line
    match = re.match(TAG_START_PAT, line)
    if match:
        return match.groups()[0].lower() in EXCLUDE_PROCESSING_TAGS
...
def indent_lines(lines,
    ...
    def recurse(prefix_lines):
        ...
        while prefix_lines:
            ...
            in_exclude_block = exclude_tag_start_check(line)

            if in_exclude_block:
                block_size = 0
                for prefix, line in prefix_lines:
                    block_size += 1
                    if exclude_tag_end_check(line):
                        break
                output.extend((prefix+line for prefix, line in prefix_lines[:block_size]))
